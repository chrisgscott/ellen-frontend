...
[2025-08-07 16:52] — Extended `/api/news` transform to include `source`, `geographic_focus`, `interest_cluster`, `type`, `related_materials`, `analysis_version`, and `analysis_completed_at`. Added robust normalization for `related_materials` (array, JSON string, or CSV) in `app/api/news/route.ts`. Updated `NewsItem` interface in `components/newsfeed-sidebar.tsx`. Enhanced `components/article-view.tsx` with meta badges (source/region/cluster/type), plain-text snippet preview (no full content), subtle analysis metadata, and Related Materials chips linking to `/home/research/[material]`. Kept `emailed` hidden as requested.
[2025-08-07 17:26] — Removed "BLUF" heading label from the highlighted summary in `components/article-view.tsx`, keeping the accent box and larger text. Proposed switching Related Materials chips to `RelatedMaterialsCard` by adding `/api/materials/by-names` to return card fields and a small client component to fetch-and-render cards with a chips fallback (pending approval).
