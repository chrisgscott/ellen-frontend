...
export function useSession(
  ...
  // Flag to track if a message send is in progress
    ...
    try {
      ...
      if (abortController) {
        try {
          ...
          // Small delay to ensure previous controller is fully aborted
          await new Promise(resolve => setTimeout(resolve, 50));
        } catch (abortErr) {
          console.warn('Error aborting previous request:', abortErr);
        }
      }
  ...
  }, [session, projectId, abortController, createSession, fetchSession, appendAssistantToken, updateSources, updateMaterials, updateSuggestions, isSending]);
