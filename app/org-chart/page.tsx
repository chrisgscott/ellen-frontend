...
export default async function OrgChartPage() {
  ...
  // Access gate: require org_chart_access flag on the user's profile
  const {
    data: { user },
  ...
  if (!user) {
    return (
      <div className="p-6">
        ...
        <p className="text-sm text-muted-foreground mt-2">You must be signed in to view this page.</p>
      </div>
    )
  }
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('org_chart_access')
    .eq('id', user.id)
    .single()

  if (!profile?.org_chart_access) {
    return (
      <div className="min-h-[100dvh] w-full flex items-center justify-center p-6">
        <div className="max-w-xl text-center">
          <h1 className="text-2xl font-semibold">Org Chart Access Required</h1>
          <p className="text-sm text-muted-foreground mt-3">
            You don’t currently have access to the organization chart. If you believe this is an error or you need access,
            please email an administrator at{' '}
            <a
              href="mailto:cscott@tier-tech.com?subject=Org%20Chart%20Access%20Request"
